...
class HerniMapa:
    ...
    def nacti_textury(self):
        ...
        # Načtení textury pro cílovou stanici (Nexus)
        try:
            img = pygame.image.load("gfx/nexus.png").convert_alpha()
            self.nexus_texture = pygame.transform.smoothscale(img, (config.VELIKOST_POLICKA, config.VELIKOST_POLICKA))
        except (pygame.error, FileNotFoundError):
            self.nexus_texture = None
    ...
    def vytvor_grid(self):
        ...
        # Zjistíme mřížkové souřadnice posledního políčka z waypointů
        last_wp = config.AKTUALNI_WAYPOINTY[-1]
        last_col = int(last_wp[0] // config.VELIKOST_POLICKA)
        last_row = int(last_wp[1] // config.VELIKOST_POLICKA)
        ...
        for radek_index in range(config.RADKU):
            ...
            for sloupec_index in range(config.SLOUPCU):
                ...
                if policko in (1, 2):
                    if policko == 2 or (sloupec_index == last_col and radek_index == last_row):
                        radek_textur.append(self.nexus_texture)
                        continue

                    up = radek_index > 0 and config.AKTUALNI_MAPA[radek_index-1][sloupec_index] in (1, 2)
                    down = radek_index < config.RADKU - 1 and config.AKTUALNI_MAPA[radek_index+1][sloupec_index] in (1, 2)
                    left = sloupec_index > 0 and config.AKTUALNI_MAPA[radek_index][sloupec_index-1] in (1, 2)
                    right = sloupec_index < config.SLOUPCU - 1 and config.AKTUALNI_MAPA[radek_index][sloupec_index+1] in (1, 2)
