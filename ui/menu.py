...
class Menu:
    def __init__(self, in_game=False):
        ...
        if not in_game:
            self.width, self.height = config.SIRKA_OKNA, config.VYSKA_OKNA
        ...
        try:
            img = pygame.image.load("gfx/Menu.png").convert()
            self.bg_image = pygame.transform.smoothscale(img, (self.width, self.height))
        except (pygame.error, FileNotFoundError):
            self.bg_image = None
        ...
        stred_x = self.width // 2 - 200

        self.play_btn = Button(stred_x, 200, 400, 60, "Play", self.font, self.DARK_GREEN, self.GREEN)
        self.settings_btn = Button(stred_x, 280, 400, 60, "Settings", self.font, self.DARK_GREEN, self.GREEN)
        self.quit_btn = Button(stred_x, 360, 400, 60, "Quit", self.font, self.DARK_GREEN, self.GREEN)
        
        self.fps_btn = Button(stred_x, 120, 400, 50, f"FPS: {config.FPS}", self.font, self.DARK_GREEN, self.GREEN)
        self.cursor_btn = Button(stred_x, 180, 400, 50, f"Cursor: {config.CURSOR_SKIN}", self.font, self.DARK_GREEN, self.GREEN)
        self.vol_music_down = Button(stred_x, 260, 50, 50, "-", self.font, self.DARK_GREEN, self.GREEN)
        self.vol_music_up = Button(stred_x + 350, 260, 50, 50, "+", self.font, self.DARK_GREEN, self.GREEN)
        self.vol_sfx_down = Button(stred_x, 320, 50, 50, "-", self.font, self.DARK_GREEN, self.GREEN)
        self.vol_sfx_up = Button(stred_x + 350, 320, 50, 50, "+", self.font, self.DARK_GREEN, self.GREEN)
        self.back_btn = Button(stred_x, 420, 400, 60, "Back", self.font, self.DARK_GREEN, self.GREEN)
        
        self.level1_btn = Button(stred_x, 200, 400, 60, "Level 1", self.font, self.DARK_GREEN, self.GREEN)
        self.level2_btn = Button(stred_x, 280, 400, 60, "Level 2", self.font, self.DARK_GREEN, self.GREEN)
    ...
    def levels_menu(self):
        while True:
            ...
            # Vykreslení stínu titulku pro lepší čitelnost
            title_shadow = self.title_font.render("Select Level", True, (0, 0, 0))
            self.screen.blit(title_shadow, (self.width // 2 - title_shadow.get_width() // 2 + 2, 102))
    ...
    def settings_menu(self):
        while True:
            ...
            self.screen.blit(title_shadow, (self.width // 2 - title_shadow.get_width() // 2 + 2, 52))
            ...
            self.screen.blit(music_txt, (self.width // 2 - 100, 270))
            ...
            self.screen.blit(sfx_txt, (self.width // 2 - 100, 330))
